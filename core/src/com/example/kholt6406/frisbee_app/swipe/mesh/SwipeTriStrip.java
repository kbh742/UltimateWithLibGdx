package com.example.kholt6406.frisbee_app.swipe.mesh;
...
public class SwipeTriStrip {
    ...
    private int generate(Array<Vector2> input, int mult) {
        ...
        if (endcap<=0) {
            ...
            perp.set(p).sub(p2).mulAdd(perp,endcap);
        ...
        for (int i=1; i<input.size-1; i++) {
            ...
            perp.mulAdd(perp, thick/2f);

            //decide on which side we are using
            perp.mulAdd(perp,mult);
        ...
        if (endcap<=0) {
            ...
            perp.set(p2).sub(p).mulAdd(perp, endcap);
